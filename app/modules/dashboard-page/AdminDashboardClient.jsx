...
import { getStatusChip } from '@/components/commons/statusColors'
...
const QUICK_LINKS = [
  { label: 'Users',        desc: 'Roles & accounts',       href: 'users',        icon: UserCog },
  { label: 'Services',     desc: 'Catalog & pricing',      href: 'services',     icon: Stethoscope },
  { label: 'Appointments', desc: 'View & manage all',      href: 'appointments', icon: CalendarDays },
  { label: 'Settings',     desc: 'Clinic profile & hours', href: 'settings',     icon: Clock },
]

function StatusDonut({ breakdown, monthName }) {
  ...
  if (total === 0) {
    return (
      <EmptyState
        ...
        description={`Nothing scheduled in ${monthName} so far.`}
      />
    )
  }
  ...
  const data = breakdown
    ...
    .map((b) => {
      const c = getStatusChip(b.status)
      return { id: b.status, value: b.count, label: c.label, color: c.color }
    })
    ...
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
      <Box sx={{ position: 'relative', width: 150, height: 150, flexShrink: 0 }}>
        ...
        <Box sx={{
          ...
          <Typography variant='h5' sx={{ lineHeight: 1 }}>{total}</Typography>
          <Typography variant='caption' sx={{ color: colors.faint }}>total</Typography>
        </Box>
      </Box>

      <Box sx={{ flex: 1, minWidth: 140, display: 'flex', flexDirection: 'column', gap: 0.75 }}>
        {data.map((d) => (
          <Box key={d.id} sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
            <Box sx={{ width: 9, height: 9, borderRadius: '3px', bgcolor: d.color, flexShrink: 0 }} />
            <Typography variant='body2' sx={{ color: colors.slateText, flex: 1 }} noWrap>{d.label}</Typography>
            <Typography variant='body2' sx={{ fontWeight: 700, color: colors.ink }}>{d.value}</Typography>
          </Box>
        ))}
      </Box>
    </Box>
...
export default function AdminDashboardClient({ session, totalUsers, totalServices, totalPatients, apptThisMonth, pending, recentAppts, statusBreakdown = [], monthName }) {
      ...
      <motion.div variants={stagger} initial='hidden' animate='visible'>
        ...
        <motion.div variants={fadeUp}>
          <Box sx={{ mb: 4 }}>
            ...
            <Typography variant='body2' sx={{ color: colors.faint, mt: 0.5 }}>
              Clinic-wide overview · {monthName} {new Date().getFullYear()}
            </Typography>
          </Box>
        </motion.div>
        ...
        <motion.div variants={fadeUp}>
          <Box sx={{
            ...
            <StatCard label='Total users'     value={totalUsers}    href='users'        icon={Users} />
            <StatCard label='Total patients'  value={totalPatients} href='patients'     icon={UserCog} />
            <StatCard label='Services'        value={totalServices} href='services'     icon={Stethoscope} />
            <StatCard label={`In ${monthName}`} value={apptThisMonth} href='appointments' icon={CalendarDays} sub='Appointments' />
            ...
            <Box sx={{
              ...
              <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                ...
                <Typography variant='body2' fontWeight={600} sx={{ color: '#92400e' }}>
                  {pending} booking request{pending > 1 ? 's' : ''} awaiting confirmation
                </Typography>
              </Box>
        ...
        {/* Bottom grid: recent (left) + analytics & quick links (right) */}
        <motion.div variants={fadeUp}>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr', lg: '1.5fr 1fr' }, gap: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
              <SectionCard title={`Appointments · ${monthName}`} icon={PieIcon}>
                <StatusDonut breakdown={statusBreakdown} monthName={monthName} />
              </SectionCard>
