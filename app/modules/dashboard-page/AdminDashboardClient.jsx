...
const QUICK_LINKS = [
  { label: 'Users',        desc: 'Roles & accounts',        href: 'users' },
  { label: 'Services',     desc: 'Catalog & pricing',       href: 'services' },
  { label: 'Appointments', desc: 'View & manage all',       href: 'appointments' },
  { label: 'Settings',     desc: 'Clinic profile & hours',  href: 'settings' },
]

export default function AdminDashboardClient({ session, totalUsers, totalServices, totalPatients, apptThisMonth, pending, recentAppts, monthName }) {
  ...
  return (
    <Box sx={{ p: { xs: 2, sm: 3 } }}>
      <motion.div variants={stagger} initial='hidden' animate='visible'>
        ...
        {/* Stats — 5 cards: 3 top + 2 bottom */}
        <motion.div variants={fadeUp}>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(3, 1fr)' }, gap: 2, mb: 2 }}>
            <StatCard label='Total users'           value={totalUsers}     href='users' />
            <StatCard label='Total patients'         value={totalPatients}  href='appointments' />
            <StatCard label='Services offered'       value={totalServices}  href='services' />
          </Box>
          <Box sx={{ display: 'grid', gridTemplateColumns: { xs: '1fr 1fr', sm: 'repeat(3, 1fr)' }, gap: 2, mb: 3 }}>
            <StatCard label={`Appointments in ${monthName}`} value={apptThisMonth} href='appointments' />
                    ...
                    return (
                      <Box
                        ...
                        sx={{
                          ...
                          transition: 'background 0.12s', '&:hover': { bgcolor: '#fafafa' },
                        }}
                      >
                        <Box sx={{
