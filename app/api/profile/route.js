...
export async function GET() {
  ...
  const user = await prisma.user.findUnique({
    ...
    select: { id: true, firstName: true, lastName: true, email: true, isDeleted: true }
  })
...
export async function PATCH(request) {
  ...
  const updated = await prisma.user.update({
    where: { id: session.userId },
    data: { firstName: firstName.trim(), lastName: lastName.trim(), email: email.trim() }, // email simple change for now
