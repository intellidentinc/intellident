...
async function main() {
  const clinic = await prisma.clinic.findFirst({ where: { code: 'KH' } });
  if (!clinic) {
    console.error('KH Dental Aesthetics (KH) not found.');
    process.exit(1);
  }
  ...
  console.log(`Created ${CAB_SERVICES.length} new service(s) for ${clinic.name}.`);
}
