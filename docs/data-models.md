...
## Patient Records
- `PatientRecord` — E2EE encrypted clinical notes per patient; fields: `patientId`, `clinicId`, `title`, `encryptedData?`, `dataIv?`, `contentHash?` (SHA-256 hash of plaintext for tamper detection — computed on every write, verified on every read; mismatch surfaces a warning), `status` (RecordStatus enum: `ACTIVE | ARCHIVED`)
- `RecordHistory` — JSON diff stored on every `PatientRecord` PATCH; fields: `recordId`, `changedById`, `diff Json` (before/after values), `changedAt`; accessible via `GET /api/records/[patientId]/[recordId]/history`
