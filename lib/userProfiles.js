...
export async function reconcileRoleProfile(tx, { userId, role, clinicId, firstName, lastName }) {
  ...
  if (role === ROLES.DENTIST) {
    ...
    const existing = await tx.patient.findUnique({ where: { userId_clinicId: { userId, clinicId } }, select: { id: true } })
    if (existing) {
      await tx.patient.update({ where: { id: existing.id }, data: { isDeleted: false, deletedAt: null } })
    } else {
