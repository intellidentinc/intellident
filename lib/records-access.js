...
export async function dentistTreatsPatient({ dentistId, patientId, clinicId }) {
  ...
  const assignment = await prisma.patientCareAssignment.findFirst({
    where: { dentistId, patientId, clinicId, isActive: true },
    select: { id: true },
  })
...
export async function getAuthorizedReaderIds({ patientId, clinicId }) {
  ...
  for (const assignment of patient.careAssignments) {
    if (assignment.dentist?.userId) readerIds.add(assignment.dentist.userId)
  }
