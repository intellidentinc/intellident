...
export async function sendCustomAppointmentEmail({ to, subject, body, typeKey }) {
  ...
  await sendMail(to, subject, body, html, { unsubscribe: true })
}
...
export async function sendAppointmentBookingEmail({ to, staffName, patientName, serviceName, scheduledAt, appointmentCode }) {
  ...
  await sendMail(to, 'New Appointment Booking Request — IntelliDent', text, html, { unsubscribe: true })
}
...
export async function sendAppointmentConfirmedEmail({ to, firstName, serviceName, scheduledAt, appointmentCode }) {
  ...
  await sendMail(to, 'Appointment Confirmed — IntelliDent', text, html, { unsubscribe: true })
}
...
export async function sendAppointmentCancelledEmail({ to, firstName, serviceName, scheduledAt, appointmentCode }) {
  ...
  await sendMail(to, 'Appointment Cancelled — IntelliDent', text, html, { unsubscribe: true })
}
...
export async function sendAppointmentCompletedEmail({ to, firstName, serviceName, scheduledAt }) {
  ...
  await sendMail(to, 'Thank You for Your Visit — IntelliDent', text, html, { unsubscribe: true })
}
...
export async function sendAppointmentNoShowEmail({ to, firstName, serviceName, scheduledAt }) {
  ...
  await sendMail(to, 'Appointment No-show — IntelliDent', text, html, { unsubscribe: true })
}
...
export async function sendAppointmentRescheduledEmail({ to, firstName, serviceName, scheduledAt, appointmentCode }) {
  ...
  await sendMail(to, 'Appointment Rescheduled — IntelliDent', text, html, { unsubscribe: true })
}
...
export async function sendAppointmentReminderEmail({ to, firstName, serviceName, scheduledAt, appointmentCode, hoursAhead }) {
  ...
  await sendMail(to, `Appointment Reminder — ${label === 'tomorrow' ? '24-Hour' : '2-Hour'} Notice — IntelliDent`, text, html, { unsubscribe: true })
}
...
export async function sendStepUpOtpEmail({ to, firstName, code }) {
  const displayName = firstName || 'there'
  const html = `
<!DOCTYPE html>
<html lang="en">
...
<body style="margin:0;padding:0;background-color:#F8FAFC;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;">
      ...
      <table width="100%" cellpadding="0" cellspacing="0" role="presentation" style="max-width:560px;background:#ffffff;border-radius:16px;box-shadow:0 4px 24px rgba(37,99,235,0.08);overflow:hidden;">
        <tr>
          <td style="background:linear-gradient(135deg,#2563eb 0%,#1d4ed8 100%);padding:40px 48px 36px;">
            <span style="font-size:22px;font-weight:800;color:#ffffff;letter-spacing:-0.5px;">Intelli<span style="color:#93c5fd;">Dent</span></span>
            <div style="margin-top:24px;width:56px;height:56px;background:rgba(255,255,255,0.15);border-radius:50%;display:inline-block;text-align:center;line-height:56px;font-size:28px;">🔒</div>
            <h1 style="margin:16px 0 0;font-size:26px;font-weight:700;color:#ffffff;">Your record access code</h1>
            <p style="margin:8px 0 0;font-size:15px;color:#bfdbfe;">Identity verification required</p>
          </td>
        </tr>
        <tr>
          <td style="padding:40px 48px 32px;">
            <p style="margin:0 0 20px;font-size:16px;color:#334155;">Hey <strong>${displayName}</strong>,</p>
            <p style="margin:0 0 28px;font-size:15px;color:#475569;line-height:1.7;">
              Use the code below to verify your identity and access patient records in <strong style="color:#2563eb;">IntelliDent</strong>.
            </p>
            <table width="100%" cellpadding="0" cellspacing="0" role="presentation">
              <tr>
                <td align="center" style="padding:8px 0 32px;">
            ...
            <p style="margin:24px 0 0;font-size:13px;color:#94a3b8;line-height:1.6;">
              <strong style="color:#64748b;">Didn't request this code?</strong> Someone may have access to your account. Please contact your clinic administrator immediately.
            </p>
          </td>
        </tr>
        <tr>
  ...
  const text = `Hey ${displayName},\n\nYour IntelliDent record access code is: ${code}\n\nThis code expires in 10 minutes. Do not share it with anyone.\n\nIf you didn't request this, please contact your clinic administrator immediately.\n\n— IntelliDent`

  await sendMail(to, 'Your IntelliDent record access code', text, html)
}

async function sendMail(to, subject, text, html, opts = {}) {
  const sender = process.env.GMAIL_USER;
  // A monitored Reply-To that matches the sending domain improves trust/alignment.
  const replyTo = process.env.GMAIL_REPLY_TO || sender;

  // Gmail/Yahoo bulk-sender rules expect a List-Unsubscribe header on
  // recurring/notification mail. Opt-in only — never on security/auth mail.
  const headers = {};
  if (opts.unsubscribe) {
  ...
  await transporter.sendMail({
    from: `"${process.env.GMAIL_FROM_NAME ?? 'IntelliDent'}" <${sender}>`,
    to,
    replyTo,
    subject,
    text,
