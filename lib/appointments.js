...
/**
 * Fetch a patient's own appointments for the My Schedules page, split by tab.
 *   upcoming → PENDING | CONFIRMED | RESCHEDULED with scheduledAt ≥ now
 *   past     → COMPLETED | CANCELLED | NO_SHOW, or any with scheduledAt < now
 * Shared by GET /api/schedules and the server-rendered schedules page so the
 * shape/ordering stays in sync.
 */
export function getPatientAppointments(patientId, tab = 'upcoming') {
  const now = new Date()
  const where = {
    patientId,
    isDeleted: false,
